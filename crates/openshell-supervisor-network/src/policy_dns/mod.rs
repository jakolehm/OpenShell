    ...
    pub(crate) async fn answer_query(
        ...
        if eligible.is_empty() {
            if self.deferred_dns_approval
                ...
                && observation_eligible(&normalized_name)
                && self
                    .policy
...
";
    ...
    #[tokio::test]
    async fn deferred_approval_keeps_reserved_names_and_quarantine_refused() {
        let service = service(BASE_POLICY, Vec::new()).with_deferred_dns_approval();
        service
            .policy
            .set_connection_approval_hook(Arc::new(DenyApproval))
            .unwrap();
        let now = Instant::now();
        for name in ["localhost", "metadata.google.internal"] {
            assert!(
                matches!(
                    service.answer_query(name, AddressFamily::Ipv4, now).await,
                    Err(PolicyDnsError::Ineligible)
    ...
    async fn deferred_dns_cannot_exhaust_the_pool_reserved_for_declared_endpoints() {
        ...
        assert!(matches!(
            service
                .answer_query("third.example", AddressFamily::Ipv4, now)
                .await,
            Err(PolicyDnsError::Publish(
                PublishError::ObservationBudgetExhausted
            ))
        ));
        assert!(
            service
