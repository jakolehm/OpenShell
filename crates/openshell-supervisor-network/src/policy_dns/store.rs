...
pub(crate) struct ResolvedEndpointRecord {
    ...
    pub(crate) contracts: Vec<ResolvedPortContract>,
    pub(crate) deferred_approval: bool,
}
...
impl ResolvedEndpointRecord {
    ...
    pub(crate) fn is_observation(&self) -> bool {
        self.contracts.is_empty() && !self.deferred_approval
    }
        ...
        self.publish_checked(
            Publication {
                key: AllocationKey {
                    normalized_name,
                    family,
                    identity: AllocationIdentity::DeferredApproval,
                },
    ...
    fn publish_checked(
        ...
        let synthetic_address = if let Some(address) = state.allocations.get(&key) {
            *address
            ...
            if !matches!(key.identity, AllocationIdentity::Contracts(_))
                && state
                    .allocations
                    .keys()
                    .filter(|allocated| {
                        !matches!(allocated.identity, AllocationIdentity::Contracts(_))
        ...
        let record = ResolvedEndpointRecord {
            ...
            expires_at: now + ttl,
            contracts,
            deferred_approval: key.identity == AllocationIdentity::DeferredApproval,
        };
