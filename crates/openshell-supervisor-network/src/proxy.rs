...
/// Explicit deferred-approval records return no pinned plan and still require destination validation.
fn pinned_transparent_plan(
    store: &ResolvedEndpointStore,
    destination: SocketAddr,
    decision: &EgressDecision,
) -> std::result::Result<Option<DestinationValidationPlan>, MappingLookupError> {
    ...
    if mapping.record.deferred_approval {
        return Ok(None);
    }
...
";
    ...
    #[tokio::test]
    async fn deferred_approval_plan_is_distinct_from_observation_and_generation_bound() {
        let engine = OpaEngine::from_strings(
            include_str!("../data/sandbox-policy.rego"),
            "network_policies: {}\n",
        )
        ...
        let record = store
            .publish_deferred(
                crate::policy_dns::NormalizedName::parse("pypi.org").unwrap(),
                crate::policy_dns::AddressFamily::Ipv4,
                generation,
                std::time::Instant::now(),
            )
            .unwrap();
        let destination = SocketAddr::new(record.synthetic_address, 80);
        let (open, _) = staged_curl_open(destination, generation);
        let decision = authorize_supplied_identity(
            &engine,
            &BinaryIdentityCache::new(),
            EgressIntent::connect("pypi.org".to_string(), 80),
            &open.binary_identity,
        );
        assert!(matches!(decision.action, NetworkAction::Deny { .. }));
        let mut decision = apply_connection_approval(
            &engine,
            &BinaryIdentityCache::new(),
            &open.binary_identity,
            decision,
        ...
        assert!(matches!(decision.action, NetworkAction::Allow { .. }));
        assert!(
            pinned_transparent_plan(&store, destination, &decision)
                .unwrap()
                .is_none()
        );
        decision.policy_generation += 1;
        assert!(matches!(
            pinned_transparent_plan(&store, destination, &decision),
            Err(MappingLookupError::StalePolicy)
        ));
        let observed = publish_observation(&store, "pypi.org", generation);
        assert_ne!(record.synthetic_address, observed.synthetic_address);
    }
